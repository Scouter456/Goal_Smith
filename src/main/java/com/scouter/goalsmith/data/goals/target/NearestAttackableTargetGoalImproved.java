...
public class NearestAttackableTargetGoalImproved extends TargetGoal {
    ...
    public NearestAttackableTargetGoalImproved(Mob pMob, TagKey<EntityType<?>> pTargetType, int pRandomInterval, boolean pMustSee, boolean pMustReach, @Nullable Predicate<LivingEntity> pTargetPredicate) {
    ...
    protected void findTarget() {
        this.target = this.mob.level().getNearestEntity(this.mob.level().getEntitiesOfClass(LivingEntity.class, this.getTargetSearchArea(this.getFollowDistance()), (entity) -> {
            return entity.getType().is(targetType);
